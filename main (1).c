...
void tostring(char str[], int num)
{
    ...
    while ( 1 )
    {
        tostring(czasSek ,czasWrt%60);
        tostring(czasMin ,czasWrt/60);
        tostring(mocWrtStr ,mocWrt);
        LCD_PutString ( moc , sizeof(moc) ) ;
        LCD_PutString ( mocWrtStr , sizeof(mocWrtStr) ) ;
        LCD_PutString ( "W" , 1 ) ;
        LCD_PutString ( czas , sizeof(czas) ) ;
        LCD_PutString ( czasMin , sizeof(czasMin) ) ;
        LCD_PutString ( ":" , 1 ) ;
        LCD_PutString ( czasSek , sizeof(czasSek) ) ;
        delay(500);
        LCD_ClearScreen();
        ...
        while((BUTTON_IsPressed(BUTTON_S5))&&(czasWrt>0))
        {
            delay(1000);
            while(czasWrt!=0)
            {
